...
// 해당월의 마지막일자꺼내기 함수
// function fCheck5() {
let fCheck5 = function() {

  let year = document.getElementById("year").value;
  let month = document.getElementById("month").value;

  let str = new Date(year,month,0); //0=마지막일자 new DAte(년,월,일,시,분,초);
...
// 오늘날짜 형식에 맞춰서 출력(yyyy-mm-dd)
function fCheck6() {
  let strDate = year + "-" + month + "-" + date;
  let fmtDate = strDate.split("-");
  let yy = fmtDate[0];
  let mm = ('0' + fmtDate[1]).slice(-2);    // 2월 --> 02월 : 02 --> 002 -> 뒤에서 2자리만꺼내면됨
  let dd = ('0' + fmtDate[2]).slice(-2);   // 2월 --> 02월 : 02 --> 002 -> 뒤에서 2자리만꺼내면됨
  strDate = yy + "-" + mm + "-" + dd;
...
function fCheck7() {
  ...
  let str = new Date(year,month,0); //0=마지막일자 new DAte(년,월,일,시,분,초);
  let strDate = str.getFullYear() + "-";
  strDate += ('0' + str.getMonth()+1).slice(-2) + "-";
  strDate += str.getDate();
  strDate = "해당월의 마지막 일자는? " + strDate;
  demo.innerHTML = strDate;
}